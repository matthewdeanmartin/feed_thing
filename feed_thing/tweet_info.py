...
def get_timeline(account, mastodon: Mastodon)->TimelineInfo:
    ...
    manager.excute_nonquery("CREATE TABLE IF NOT EXISTS timeline (id INTEGER, data TEXT);")
...
def get_preexisting_timeline_info() -> TimelineInfo:
    db_objects = [(uid, json.loads(x)) for uid, x in manager.select_sql("SElect id, data from timeline")]
    user_info =  TimelineInfo(
        tweets=[x[1] for x in db_objects],
        tweets_ids=set(x[0] for x in db_objects)
    )
    return user_info
